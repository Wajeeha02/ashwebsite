import {
  ...
  q,
  x
} from "./chunk-B3ZUQSBL.js";
import {
  G,
  _,
  b,
  k
} from "./chunk-B3TW2DAA.js";
...
function useLocation() {
  ...
  return P(LocationContext).location;
}
function useNavigationType() {
  return P(LocationContext).navigationType;
}
function useMatch(pattern) {
  ...
  return q(() => matchPath(pattern, pathname), [pathname, pattern]);
}
var navigateEffectWarning = "You should call navigate() in a React.useEffect(), not when your component is first rendered.";
function useIsomorphicLayoutEffect(cb) {
  let isStatic = P(NavigationContext).static;
  if (!isStatic) {
    A(cb);
  }
}
...
function useNavigateUnstable() {
  ...
  let dataRouterContext = P(DataRouterContext);
  let {
    basename,
    future,
    navigator
  ...
  let activeRef = F(false);
  useIsomorphicLayoutEffect(() => {
    activeRef.current = true;
  });
  let navigate = x(function(to, options) {
    if (options === void 0) {
      options = {};
    }
...
function useOutletContext() {
  return P(OutletContext);
}
function useOutlet(context) {
  let outlet = P(RouteContext).outlet;
  if (outlet) {
...
function useResolvedPath(to, _temp2) {
  ...
  let routePathnamesJson = JSON.stringify(getResolveToMatches(matches, future.v7_relativeSplatPath));
  return q(() => resolveTo(to, JSON.parse(routePathnamesJson), locationPathname, relative === "path"), [to, routePathnamesJson, locationPathname, relative]);
}
...
function useDataRouterContext(hookName) {
  let ctx = P(DataRouterContext);
  !ctx ? true ? invariant(false, getDataRouterConsoleError(hookName)) : invariant(false) : void 0;
  return ctx;
}
function useDataRouterState(hookName) {
  let state = P(DataRouterStateContext);
  !state ? true ? invariant(false, getDataRouterConsoleError(hookName)) : invariant(false) : void 0;
  return state;
}
function useRouteContext(hookName) {
  let route = P(RouteContext);
...
function useMatches() {
  ...
  return q(() => matches.map((m) => convertRouteMatchToUiMatch(m, loaderData)), [matches, loaderData]);
}
...
function useBlocker(shouldBlock) {
  ...
  let [blockerKey, setBlockerKey] = p("");
  let blockerFunction = x((arg) => {
...
function MemoryRouter(_ref3) {
  ...
  let setState = x((newState) => {
    v7_startTransition && startTransitionImpl ? startTransitionImpl(() => setStateImpl(newState)) : setStateImpl(newState);
  }, [setStateImpl, v7_startTransition]);
  A(() => history.listen(setState), [history, setState]);
  return _(Router, {
    basename,
    children,
    location: state.location,
...
function Navigate(_ref4) {
  ...
  _2(() => navigate(JSON.parse(jsonPath), {
    replace,
    state,
    relative
...
function RouterProvider(_ref) {
  ...
  let [state, setStateImpl] = p(router.state);
  let [pendingState, setPendingState] = p();
  let [vtContext, setVtContext] = p({
    isTransitioning: false
  });
  let [renderDfd, setRenderDfd] = p();
  let [transition, setTransition] = p();
  let [interruption, setInterruption] = p();
  let fetcherData = F(/* @__PURE__ */ new Map());
  let {
    v7_startTransition
  } = future || {};
  let optInStartTransition = x((cb) => {
  ...
  A(() => router.subscribe(setState), [router, setState]);
  _2(() => {
  ...
  }, state.initialized || router.future.v7_partialHydration ? _(DataRoutes, {
...
var NavLink = k2(function NavLinkWithRef(_ref8, ref) {
  ...
  let routerState = P(DataRouterStateContext);
  let {
    navigator,
    basename
  } = P(NavigationContext);
...
function useSearchParams(defaultInit) {
  ...
  let defaultSearchParamsRef = F(createSearchParams(defaultInit));
  let hasSetSearchParamsRef = F(false);
  let location = useLocation();
  let searchParams = q(() => (
  ...
  let navigate = useNavigate();
  let setSearchParams = x((nextInit, navigateOptions) => {
...
function useSubmit() {
  ...
  return x(function(target, options) {
    if (options === void 0) {
      options = {};
    }
...
function useFormAction(action, _temp2) {
  ...
  let routeContext = P(RouteContext);
...
function useFetcher(_temp3) {
  ...
  let fetcherData = P(FetchersContext);
  let route = P(RouteContext);
  ...
  let [fetcherKey, setFetcherKey] = p(key || defaultKey);
  ...
  let fetcherWithComponents = q(() => _extends3({
    Form: FetcherForm,
    submit,
    load
  }, fetcher, {
...
function useViewTransitionState(to, opts) {
  ...
  let vtContext = P(ViewTransitionContext);
